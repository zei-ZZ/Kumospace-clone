...
export class LayoutComponent implements OnInit, OnDestroy {
  ...
  mapX = computed(() => Math.max(0, this.charX() - window.innerWidth / 2));
  mapY = computed(() => Math.max(0, this.charY() - window.innerHeight / 2));
  ...
  moveCharacter(direction: 'up' | 'down' | 'left' | 'right') {
    ...
    switch (direction) {
      ...
      case 'up':
        ...
        break;
      case 'down':
        this.charY.update((y) => y + step);
        break;
      case 'left':
        this.charX.update((x) => Math.max(x - step, 0));
        break;
      case 'right':
        this.charX.update((x) => x + step);
        break;
    }
  }
